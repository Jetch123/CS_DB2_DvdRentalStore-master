...
namespace DVDRentalStore.Models
{
	public class Actors
	{
		...
		public Actors(int id, string firstname, string lastname, DateTime birthday)
		{
			this.Id = id;
		...
		public static IEnumerable<Actors> GetAll()
		{
			...
			using (NpgsqlConnection conn = new NpgsqlConnection(SysConfig.CONNECTION_STRING))
			{
				...
				using (var command = new NpgsqlCommand($"SELECT * FROM {table}", conn))
				{
					...
					while (reader.Read())
						actors.Add(new Actors((int)reader["actor_id"], (string)reader["first_name"], (string)reader["last_name"], (DateTime)reader["birthday"]));
		...
		public static Actors GetByID(int id)
		{
			using (NpgsqlConnection conn = new NpgsqlConnection(SysConfig.CONNECTION_STRING))
			{
				...
				using (var command = new NpgsqlCommand($"SELECT * FROM {table} WHERE actor_id = @Id", conn))
				{
					...
					{
						reader.Read();
						return new Actors(id, (string)reader["first_name"], (string)reader["last_name"], (DateTime)reader["birthday"]);
					}
				}
			}
			return null;
